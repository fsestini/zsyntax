...
type DTSequent term eb cs a l = DT term (NeutralSequent eb cs a l)
...
type DTMatchResult term eb cs actcase a l = DT term (MatchResult eb cs actcase a l)
...
type DTFocMatchResult term eb cs a l = DT term (FocMatchRes eb cs a l)
...
positiveFocalDispatch formula =
  case formula of
    Atom a ->
      return (DT (init @_ @eb @cs @_ @l a) (MREmptyGoal mempty (singletonCtxt (NF formula))))
