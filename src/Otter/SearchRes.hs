...
-- found, or the search was terminated preemptively according to an upper bound
-- imposed on the maximum depth of the search space.
data SpaceExhausted = Exhausted | NotExhausted
...
data Extraction a = AllResults (NonEmpty a) | NoResults SpaceExhausted

resListUntil :: Int -> SearchRes a -> ([a], SpaceExhausted)
resListUntil _ [] = ([], Exhausted)
resListUntil 0 (_ : _)  = ([], NotExhausted)
