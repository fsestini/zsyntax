...
newtype ElementaryBase a = EB (S.Set (BioFormula a)) deriving (Monoid)
newtype ControlSet a = CS (S.Set (BioFormula a)) deriving (Monoid)

bsAsSet :: ElementaryBase a -> S.Set (BioFormula a)
bsAsSet (EB bs) = bs

csAsSet :: ControlSet a -> S.Set (BioFormula a)
csAsSet (CS cs) = cs

bsSingleton :: BioFormula a -> ElementaryBase a
bsSingleton f = EB (S.singleton f)
